...
Build the foundation of an Indeed-like recruitment platform with three primary roles:
...
The Job Seeker is the primary user who searches and applies for jobs.
...
The user owns one reusable professional profile.
...
### Smart Resume Reusability

If the user uploads a resume while applying for a job

Instead of keeping it only inside that application

System automatically saves it inside

My Resume Library

Later

When applying to another job
...
# B. Employer (High-Level for Phase 1)
...
# C. Admin (High-Level for Phase 1)
...
Design a scalable, normalized database schema for an Indeed-like job portal that supports reusable resumes, advanced job search, employer management, AI recommendations, reporting, messaging, and future expansion.
...
One company may have multiple offices.
...
Every job application can reuse it
...
users
 ...
 ├── 1:1 employer_profiles
 ├── 1:N resumes
 ├── 1:N experiences
 ├── 1:N education
 ├── 1:N certifications
 ├── 1:N projects
 ├── N:M skills (via user_skills)
 ├── 1:N applications
 ├── 1:N saved_jobs
 ├── 1:N job_alerts
 ├── 1:N notifications
 ├── 1:N messages
 └── N:M companies (via company_followers)
...
jobs
 ├── N:1 companies
 ├── N:1 categories
 ├── N:M skills (via job_skills)
 ├── 1:N applications
 ├── 1:N job_reports
 └── N:M users (via saved_jobs)

applications
 ├── N:1 jobs
 ├── N:1 users (applicants)
 ├── N:1 resumes
...
* Use UUIDs or BIGINT IDs consistently across all tables.
* Add `created_at`, `updated_at`, and optional `deleted_at` (soft deletes) to major entities.
* Store flexible filters, search criteria, and notification payloads in JSON columns where appropriate.
* Add indexes on high-traffic fields such as `email`, `company_id`, `category_id`, `status`, `location`, `published_at`, and `application_deadline`.
* Use foreign key constraints to maintain referential integrity.
* Keep lookup data (employment types, workplace types, experience levels, currencies, etc.) in dedicated lookup tables or enums to simplify maintenance and localization.

-----------------------------------------
Phase 3: # Phase 3: Authentication & User Roles (Week 1–2)

## Goal

Build a secure authentication and authorization system that supports multiple user roles, protects application routes, verifies user identities, and provides complete profile management for both Job Seekers and Employers.
...
Users choose their account type during registration.
...
* Assign employer to company
* Send verification email
* Company status defaults to **Pending Verification** (optional approval workflow)
...
Accessible only to authenticated Job Seekers.
...
Accessible only to authenticated Employers.
...
# 4. Job Seeker Profile Management

A reusable professional profile that supports one-click job applications.
...
* Preview Resume
* Set Default Resume
* Reuse Resume for Future Applications
...
Progress indicator (0–100%) based on profile completeness.
...
Only verified companies may publish jobs if moderation is enabled.
...
* Secure signed verification links
...
* Enable/Disable Two-Factor Authentication (Future)
* Delete Account (Soft Delete)
* Download Personal Data (GDPR-ready)
...
* Secure file upload validation for resumes and logos
...
* Complete Employer and Company profile management
* Resume library with reusable resumes
* Skills, experience, education, certifications, and projects management
* Company branding (logo, cover image, description)
